...
<html>
    ...
    <body>
        ...
        <div class="container py-4">
                <h1 class="display-1 text-primary text-center mb-4">Catalog</h1>
            <div class="row">
            <?php 
                 include("db_connect.php");
                        
                $query = "SELECT brand.name AS bname, brand.bid, product.* FROM product, brand WHERE product.bid = brand.bid ORDER BY product.pid ASC";
                $result = mysqli_query($conn,$query);
                while($row = mysqli_fetch_array($result))
                { //while open
            ?>   
                <div class="col-md-4" title="<?php echo $row['name'] ?>">
                    <div class="card m-4 shadow bg-primary">
